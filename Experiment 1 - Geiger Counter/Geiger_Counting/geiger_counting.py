...
def geiger(replicas=2, intervals=10, period=0.2, savefile=datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + "_COUNTING_DATA",graphics=True):
    ...
    if savefile and len(savefile) > 0:
        numpy.savetxt(savefile,h)
        print("histogram saved to disk as '%s'."%(savefile))
    return h
            
rc = geiger(replicas=2,intervals=100,period=0.2,graphics=True)
